...
##można tu przy okazji machnąć summary dla tych co mają 6 pokoi (n = 65) kodem - dane_do_wiz %>% filter(rooms == 6) %>% summary()
...
## Cena wynajmu w zależności od szerokości geograficznej (chyba lepiej po prostu od konkretnego miasta robić) 
...
ggplot(dane_do_wiz, aes(x = as.factor(floorCount), y = price)) +
...
## Windowo
ggplot(dane_do_wiz, aes(x = as.factor(hasElevator), y = price)) +
  geom_boxplot(fill = "cyan") +
  labs(title = "Cena wynajmu a dostępność windy", x = "Czy jest winda?", y = "Cena wynajmu") +
  coord_flip()

##Ochronowo
ggplot(dane_do_wiz, aes(x = as.factor(hasSecurity), y = price)) +
  geom_boxplot(fill = "cyan") +
  labs(title = "Cena wynajmu a istnienie ochrony", x = "Czy jest ochrona?", y = "Cena wynajmu") +
  coord_flip()

##Piwniocowo(?)
ggplot(dane_do_wiz, aes(x = as.factor(hasStorageRoom), y = price)) +
  geom_boxplot(fill = "cyan") +
...
ggplot(dane_do_wiz, aes(x = interaction(hasBalcony, hasParkingSpace, hasElevator, hasSecurity, hasStorageRoom), y = price)) +
  geom_boxplot(fill = "lightblue") +
  labs(
...
ggplot(dane_do_wiz, aes(x = city, fill = as.factor(hasParkingSpace))) +
