...
## Limitowanie osi x do 12500 [same]
ggplot(dane_do_wiz_wwa, aes(x = price)) +
  geom_histogram(binwidth = 500, fill = "blue", color = "black") +
  xlim(0, 12500) +
  labs(title = "Histogram Ceny wynajmu mieszkań", x = "Cena wynajmu", y = "Liczba mieszkań")
...
##można tu przy okazji machnąć summary dla tych co mają 6 pokoi (n = 65) kodem - dane_do_wiz_wwa %>% filter(rooms == 6) %>% summary()


## Boxplot Ceny wynajmu (tak dla hecy) [czy ograniczyć dane do 7500? 13000?]
ggplot(dane_do_wiz_wwa, aes(x = "", y = price)) +
...
## Cena wynajmu w zależności od szerokości geograficznej 
ggplot(dane_do_wiz_wwa, aes(x = longitude, y = latitude, color = price)) +
...
ggplot(dane_do_wiz_wwa, aes(x = as.factor(floorCount), y = price)) +
...
ggplot(dane_do_wiz_wwa, aes(x = schoolDistance, y = price)) +
...
ggplot(dane_do_wiz_wwa, aes(x = city, y = price)) +
...
ggplot(dane_do_wiz_wwa, aes(x = price, fill = type)) +
...
ggplot(dane_do_wiz_wwa, aes(x = rooms, y = price, color = condition)) +
...
## Cena wynajmu w zależności od odległości od centrum w podziale na miasta (naajs stufffff)
...
ggplot(dane_do_wiz_wwa, aes(x = interaction(hasBalcony, hasParkingSpace, hasElevator, hasSecurity, hasStorageRoom), y = price)) +
  geom_boxplot(fill = "lightblue") +
  labs(
...
avg_price_year <- dane_do_wiz_wwa %>%
  group_by(buildYear) %>%
  summarise(mean_price = mean(price, na.rm = TRUE))
...
ggplot(dane_do_wiz_wwa, aes(x = poiCount)) +
...
dane_do_wiz_wwa_POI <- dane_do_wiz_wwa %>%
...
ggplot(dane_do_wiz_wwa_POI, aes(x = poi_bins, y = price)) +
  geom_boxplot(fill = "lightblue") +
  labs(
...
ggplot(dane_do_wiz_wwa, aes(x = factor(floor), y = price, color = type)) +
  geom_boxplot() +
  labs(
...
ggplot(dane_do_wiz_wwa, aes(x = price, fill = cut(pharmacyDistance, breaks = 5))) +
...
ggplot(dane_do_wiz_wwa, aes(x = kindergartenDistance, y = price)) +
...
ggplot(dane_do_wiz_wwa, aes(x = collegeDistance, y = price)) +
  geom_point(alpha = 0.5, color = "blue") +
  geom_smooth(method = "loess", color = "red", se = FALSE) +
  labs(
